...
public class DAO {
	...
	//Get Account
	 ...
	 public ArrayList<OrderLine> getAllOrderLineByOid(int oid ){
	        ...
	        String query = "select Orders.id,Book.id, Book.image,Book.title,Book.author,Category.name,Book.price ,OrderLine.quantity,OrderLine.totalMoney\r\n"
	        ...
	        try {
	            ...
	            rs = ps.executeQuery();
	            while (rs.next()) {
	                list.add(new OrderLine(rs.getInt(1),rs.getInt(2),rs.getString(3), rs.getString(4), 
